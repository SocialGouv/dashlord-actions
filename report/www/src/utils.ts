export const smallUrl = (url: string): string => url
  .toLowerCase()
  .replace(/^https?:\/\//, '')
  .replace(/\/$/, '');

export const getHostName = (url: string): string => url
  .replace(/^https?:\/\//, '')
  .replace(/\/$/, '')
  .replace(/^([^/]+)\/.+$/, '$1');

export const getLastUrlSegment = (url: string): string => url.substring(url.lastIndexOf('/') + 1);
